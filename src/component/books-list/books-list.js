...
export const BooksList = () => {
    ...
    const fetchUserBook = async (username) => {
        const { books } = await BackendApi.user.getBorrowBook(username)
    ...
    const deleteBook = () => {
        if (activeBookIsbn && books.length) {
            BackendApi.book.deleteBook(activeBookIsbn).then(({ success }) => {
                ...
                NotificationManager.success("Book Deleted successfully","",1000)
                setOpenModal(false)
                setActiveBookIsbn("")
            })
        }
    ...
    useEffect(() => {
        fetchBooks().catch(console.error)
        if(user){const userrrr=JSON.parse(window.localStorage.getItem('userrr'))
        const username = (userrrr.username);
       
        
;        fetchUserBook(username).catch(console.error)}
